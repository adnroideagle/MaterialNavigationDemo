...
public class MaterialSubheader {
	...
	public MaterialSubheader(Context ctx) {
		...
		paramsText.setMargins((int) (16 * density), 0, (int) (16 * density),
				(int) (4 * density));
		...
		TypedArray values = theme.obtainStyledAttributes(typedValue.resourceId,
				R.styleable.MaterialSubheader);
		try {
			titleColor = values.getColor(
					R.styleable.MaterialSubheader_subheaderTitleColor, 0x000);
		} finally {
			values.recycle();
		}
